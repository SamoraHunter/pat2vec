...
def get_current_pat_drugs(current_pat_client_id_code, target_date_range, pat_batch, config_obj=None, cohort_searcher_with_terms_and_search=None):
    ...
    # Drugs
    if batch_mode:
        drugs = filter_dataframe_by_timestamp(pat_batch, start_year, start_month, end_year, end_month, start_day, end_day, 'order_entered')
    else:
        drugs = cohort_searcher_with_terms_and_search(
            ...
            search_string="order_typecode:\"medication\" AND " + f'updatetime:[{start_year}-{start_month}-{start_day} TO {end_year}-{end_month}-{end_day}]'
        )
