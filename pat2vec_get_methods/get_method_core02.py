...
#         raise ValueError("config_obj cannot be None. Please provide a valid configuration. get_core_02")

#     if cohort_searcher_with_terms_and_search is None:
#         raise ValueError("cohort_searcher_with_terms_and_search cannot be None. Please provide a valid configuration. get_core_02")
#     print("len(pat_batch)", len(pat_batch))
#     print(pat_batch.columns)
...
#     print(len(features))
#     print(len(features_data['observation_valuetext_analysed'].dropna()))
...
def get_core_02(current_pat_client_id_code, target_date_range, pat_batch, config_obj=None, cohort_searcher_with_terms_and_search=None):
    ...
    if(batch_mode):
        ...
        current_pat_raw = cohort_searcher_with_terms_and_search(index_name="observations", 
                                                            ...
                                                            search_string=f"obscatalogmasteritem_displayname:(\"{search_term}\") AND " + f'observationdocument_recordeddtm:[{start_year}-{start_month}-{start_day} TO {end_year}-{end_month}-{end_day}]')
