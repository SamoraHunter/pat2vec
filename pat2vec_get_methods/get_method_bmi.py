...
def get_bmi_features(current_pat_client_id_code, target_date_range, pat_batch, config_obj=None, cohort_searcher_with_terms_and_search=None):
    ...
    if batch_mode:
        ...
        current_pat_raw_bmi = cohort_searcher_with_terms_and_search(
            ...
            search_string="obscatalogmasteritem_displayname:(\"OBS BMI\" OR \"OBS Weight\" OR \"OBS Height\") AND " + f'observationdocument_recordeddtm:[{start_year}-{start_month}-{start_day} TO {end_year}-{end_month}-{end_day}]'
        )

    if len(current_pat_raw_bmi[current_pat_raw_bmi['obscatalogmasteritem_displayname'] == 'OBS BMI Calculation']) == 0:
        bmi_features = pd.DataFrame(data={'client_idcode': [current_pat_client_id_code]})
    else:
        # Get BMI features
