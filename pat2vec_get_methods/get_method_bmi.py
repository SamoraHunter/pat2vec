...
def get_bmi_features(current_pat_client_id_code, target_date_range, pat_batch, config_obj=None, cohort_searcher_with_terms_and_search=None):
    """
    ...
    if len(current_pat_raw_bmi[current_pat_raw_bmi['obscatalogmasteritem_displayname'] == 'OBS BMI Calculation']) == 0:
        ...
        # Get BMI features
        bmi_sample = current_pat_raw_bmi[current_pat_raw_bmi['obscatalogmasteritem_displayname']
                                         == 'OBS BMI Calculation']
        bmi_sample = bmi_sample[(bmi_sample['observation_valuetext_analysed'].astype(
            float) < 200) & (bmi_sample['observation_valuetext_analysed'].astype(float) > 6)]
        ...
        if len(bmi_sample) > 0:
            ...
            bmi_features[f'{term}_high'] = int(
                bool(value_array.median() > 24.9))
            bmi_features[f'{term}_low'] = int(
                bool(value_array.median() < 18.5))
            bmi_features[f'{term}_extreme'] = int(
        ...
        height_sample = height_sample[(height_sample['observation_valuetext_analysed'].astype(
            float) < 300) & (height_sample['observation_valuetext_analysed'].astype(float) > 30)]

        term = 'height'
        if len(height_sample) > 0:
            value_array = height_sample['observation_valuetext_analysed'].astype(
                float)
        ...
        weight_sample = weight_sample[(weight_sample['observation_valuetext_analysed'].astype(
            float) < 800) & (weight_sample['observation_valuetext_analysed'].astype(float) > 1)]

        term = 'weight'
        if len(weight_sample) > 0:
            value_array = weight_sample['observation_valuetext_analysed'].astype(
                float)
