...
def get_current_pat_bloods(current_pat_client_id_code, target_date_range, pat_batch, batch_mode=False, cohort_searcher_with_terms_and_search=None, config_obj=None):
    ...
    if batch_mode:
        current_pat_bloods = filter_dataframe_by_timestamp(
            pat_batch, start_year, start_month, end_year, end_month, start_day, end_day, 'basicobs_entered')
    else:
    ...
    if batch_mode:
        ...
        current_pat_bloods['datetime'] = pd.to_datetime(
            current_pat_bloods['basicobs_entered'], errors='coerce')
    ...
    basicobs_itemname_analysed_df_dict = {
        elem: current_pat_bloods[current_pat_bloods.basicobs_itemname_analysed == elem] for elem in basicobs_itemname_analysed_list}
    ...
    for j in (range(0, len(basicobs_itemname_analysed_list))):
        ...
        if (df_len >= 2):
            ...
            # recent
            agg_val = filtered_df.sort_values(
                by='datetime').iloc[-1]['basicobs_value_numeric']

            df_unique_filtered.at[i, col_name+"_most-recent"] = agg_val

            # earliest-test
            agg_val = filtered_df.sort_values(
                by='datetime').iloc[0]['basicobs_value_numeric']
        ...
        if (df_len >= 3):
           # try:
            ...
            col_name_mean = basicobs_itemname_analysed_df_dict.get(
                col_name).basicobs_value_numeric._get_numeric_data().mean()
            col_name_std = basicobs_itemname_analysed_df_dict.get(
                col_name).basicobs_value_numeric._get_numeric_data().std()
