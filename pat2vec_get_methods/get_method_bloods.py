...
def get_current_pat_bloods(current_pat_client_id_code, target_date_range, pat_batch, config_obj=None, cohort_searcher_with_terms_and_search=None):
    """
    ...
    if batch_mode:
        current_pat_bloods = filter_dataframe_by_timestamp(pat_batch, start_year, start_month, end_year, end_month, start_day, end_day, 'basicobs_entered')
    else:
        current_pat_bloods = cohort_searcher_with_terms_and_search(
            ...
            search_string="basicobs_value_numeric:* AND " + f'updatetime:[{start_year}-{start_month}-{start_day} TO {end_year}-{end_month}-{end_day}]'
        )

    if batch_mode:
        current_pat_bloods['datetime'] = current_pat_bloods['basicobs_entered'].copy()
    else:
        current_pat_bloods['datetime'] = pd.to_datetime(current_pat_bloods['basicobs_entered'], errors='coerce')
