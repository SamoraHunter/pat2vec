...
class CogStack(object):
    ...
    def cogstack2df(self, query: Dict, index: str, column_headers=None, es_gen_size: int=800, request_timeout: int=300):
        ...
        if column_headers:
            df_headers = ['_index', '_id', '_score']#['_index', '_type', '_id', '_score']
...
print("""******Watcher connected to ES Cluster!******
...
def list_chunker(entered_list):
    if len(entered_list) >= 10000:
        chunks = [entered_list[x:x+10000] for x in range(0, len(entered_list), 10000)]
    return chunks

def cohort_searcher_with_terms_and_search(index_name, fields_list, term_name, entered_list, search_string):
    if len(entered_list) >= 10000:
        results = []
        chunked_list = list_chunker(entered_list)
        for mini_list in chunked_list:
            query = {"from": 0, "size": 10000, "query": {"bool": {"filter": {"terms": {term_name: mini_list}},"must": [{"query_string": {"query": search_string}}]}},"_source": fields_list}
        ...
        query = {"from": 0,"size": 10000,"query": {"bool": {"filter": {"terms": {term_name: entered_list}},"must": [{"query_string": {"query": search_string}}]}},"_source": fields_list}
        df = cs.cogstack2df(query=query, index=index_name, column_headers=fields_list) 
        return df   
...
def cohort_searcher_no_terms(index_name, fields_list, search_string):
    query = {"from": 0,"size": 10000,"query": {"bool": {"must": [{"query_string": {"query": search_string}}]}},"_source": fields_list}
    df = cs.cogstack2df(query=query, index=index_name, column_headers=fields_list) 
    return df           

def nearest(date: datetime, lookup_dates_and_values: pd.DataFrame, date_col: str, value_col: str, 
            max_time_before: timedelta=timedelta(weeks=6), max_time_after: timedelta=timedelta(weeks=50)):
    timebefore = date - max_time_before
    timeafter = date + max_time_after
    lookup_dates_and_values = lookup_dates_and_values[(lookup_dates_and_values[date_col] > timebefore) &
                                                      (lookup_dates_and_values[date_col] < timeafter)]
    if lookup_dates_and_values.shape[0] == 0:
        return None
    min_date = min(lookup_dates_and_values.iterrows(), key=lambda x: abs(x[1][date_col] - date))
    return min_date[1][value_col]


def matcher(data_template_df, lab_results_df, source_patid_colname, source_date_colname, result_date_colname, result_testname, result_resultname, before, after): 
    data_template = data_template_df # Upload the template and inspect then rename cols, remove nulls and reset index
    data_template = data_template.dropna(subset = [source_date_colname]).reset_index(drop=True)
    data_template[source_date_colname] = pd.to_datetime(data_template[source_date_colname], utc=True)
    lab_results = lab_results_df # Import the test results      
    lab_results[result_date_colname] = pd.to_datetime(lab_results[result_date_colname], utc=True)

    bloods_filter = list(lab_results[result_testname].unique())
    bloods_values = defaultdict(list) #Function that searches for all tests per patient and then returns the closest result to the date range of each patient in the template file
    for indx, row in data_template.iterrows():
        ...
        h_id_bloods = lab_results[lab_results[source_patid_colname] == h_id] # Patient ID in results table
        for blood_code_type, sub_df in \
            h_id_bloods[h_id_bloods[result_testname].isin(bloods_filter)].groupby(result_testname): # Groups the results by blood test name
            date_val_idx = sub_df.columns.tolist().index(result_date_colname) # Organizes them by blood test date
            vals[blood_code_type] = nearest(target_time, sub_df, result_date_colname, result_resultname, # Selects the nearest blood test to display
                                            max_time_before, max_time_after)
...
def bulk_str_findall(target_colname_regex_pairs, source_colname, df_name):
    for key, value in target_colname_regex_pairs.items():
        df_name[key] = df_name[source_colname].str.lower().str.findall(value).str.join(',\n')

def bulk_str_extract(target_colname_regex_pairs, source_colname, df_name, expand):
    for key, value in target_colname_regex_pairs.items():
        df_name[key] = df_name[source_colname].str.lower().str.extract(pat = value, expand=expand, flags=re.IGNORECASE)
...
def bulk_str_extract_round_robin(target_dict, df_name, source_colname, expand):
    for key, value in target_dict.items():
        ...
        df_name[key] = df_name[source_colname].str.lower().str.extract(pat = f'{value}(.*?)({remaining_strings})', expand=expand, flags=re.IGNORECASE)[0]
...
def appendAgeAtRecord(dataFrame):
    ...
    def ageAtRecord(row):
        ...
        updateTime = datetime.strptime(row['updatetime'].split(".")[0], "%Y-%m-%dT%H:%M:%S").date()
...
def get_demographics(patlist):
    demo = cohort_searcher_with_terms_no_search(index_name="epr_documents", 
...
def pull_and_write(index_name, fields_list, term_name, entered_list, search_string):
    ...
    df_write = cohort_searcher_with_terms_and_search(index_name=index_name, 
...
def cohort_searcher_with_terms_and_search_multi(index_name, fields_list, term_name, entered_list, search_string ):
    file_name = 'temp_search_store.csv'
    ...
    if __name__ == '__main__':
        print(f"starting..")
        for _ in tqdm(pool.imap(pull_and_write, pat_list_master), total=len(pat_list_master)):
            pass
...
def iterative_multi_term_cohort_searcher_no_terms_fuzzy(terms_list, treatment_doc_filename, start_year, start_month, end_year, end_month, overwrite=True, debug=False, uuid_column_name='client_idcode'):
    if not terms_list:
    ...
    if file_exists and not overwrite:
        ...
        if(debug):
            print(f"n_unique {uuid_column_name} : {len(docs[uuid_column_name].unique())}/{len(docs)}")
