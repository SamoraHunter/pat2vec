...
def parse_meta_anns(meta_anns):
    ...
    subject_confidence = meta_anns.get(
        'Subject/Experiencer', {}).get('confidence')
...
def get_pat_document_annotation_batch(current_pat_client_idcode, pat_batch, cat=None, config_obj=None, t=None):
    ...
    multi_annots_to_df(current_pat_client_idcode, pat_batch,
                       multi_annots, config_obj=config_obj, t=t)
...
def get_pat_document_annotation_batch_mct(current_pat_client_idcode, pat_batch, cat=None, config_obj=None, t=None):
    ...
    multi_annots = annot_pat_batch_docs(current_pat_client_idcode=current_pat_client_idcode, pat_batch=pat_batch,
                                        cat=cat, config_obj=config_obj, t=t, text_column='observation_valuetext_analysed')

    # creaet the file in its dir
    multi_annots_to_df_mct(current_pat_client_idcode, pat_batch, multi_annots, config_obj=config_obj, t=t,
                           text_column='observation_valuetext_analysed', time_column='observationdocument_recordeddtm', guid_column='observation_guid')
...
def get_pat_document_annotation_batch_reports(current_pat_client_idcode, pat_batch, cat=None, config_obj=None, t=None):

    # get the annotations for the pat documents
    multi_annots = annot_pat_batch_docs(current_pat_client_idcode=current_pat_client_idcode, pat_batch=pat_batch,
                                        cat=cat, config_obj=config_obj, t=t, text_column='body_analysed')

    # creaet the file in its dir
    multi_annots_to_df_reports(current_pat_client_idcode, pat_batch, multi_annots, config_obj=config_obj, t=t,
                           text_column='body_analysed', time_column='updatetime', guid_column='basicobs_guid')
...
def annot_pat_batch_docs(current_pat_client_idcode, pat_batch, cat=None, config_obj=None, t=None, text_column='body_analysed'):
    ...
    multi_annots = cat.get_entities_multi_texts(
        pat_batch[text_column].dropna())
...
def multi_annots_to_df_reports(current_pat_client_idcode,
                       pat_batch, multi_annots,
                       config_obj=None, t=None,
                       text_column='body_analysed',
                       time_column='updatetime',
    ...
    pre_document_annotation_batch_path = config_obj.pre_document_annotation_batch_path_reports

    current_pat_document_annotation_batch_path = os.path.join(
        pre_document_annotation_batch_path, current_pat_client_idcode + ".csv")

    update_pbar(current_pat_client_idcode, start_time, 5, 'multi_annots_to_df_reports',
                n_docs_to_annotate=n_docs_to_annotate,
                t=t, config_obj=config_obj)
...
def calculate_pretty_name_count_features(df_copy, suffix="epr"):
    ...
    if (len(df_copy) > 0):
        ...
        result_vector = df_copy.groupby(
            'pretty_name').size().reset_index(name='count')
        ...
        for feature_name, (column, function) in additional_features.items():
            result_vector[feature_name] = df_copy.groupby(
                'pretty_name')[column].agg(function)
