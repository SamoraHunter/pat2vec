...
def generate_pie_charts(all_batch_annots, save_plots=True, types=None):
    ...
    for client_id in unique_clients:
        ...
        cui_counts.loc[cui_counts.index.isin(bottom_25_values)] = cui_counts[~cui_counts.index.isin(bottom_25_values)].sum()
        ...
        if save_plots:
            output_filename_all_types = os.path.join(output_folder, f'pie_chart_all_types_client_{client_id}.png')
        ...
        for ctype in types:
            ...
            type_bottom_25_values = type_counts[type_counts <= type_bottom_25_threshold].index

            # Group the bottom 25% into "other" for each type
            type_counts.loc[type_counts.index.isin(type_bottom_25_values)] = type_counts[~type_counts.index.isin(type_bottom_25_values)].sum()
            type_counts = type_counts[~type_counts.index.isin(type_bottom_25_values)]
            type_counts['other'] = type_counts[type_counts.index.isin(type_bottom_25_values)].sum()
            ...
            # Save the plot if specified
            if save_plots:
                output_filename_type = os.path.join(output_folder, f'pie_chart_type_{ctype}_client_{client_id}.png')
