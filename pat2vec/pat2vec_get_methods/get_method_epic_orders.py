...
def search_epic_orders(
    ...
    output_filename: str | None = "epic_orders_results.csv",
    ...
    update_pbar(
        ...
        stage_int=0,
        stage_str="epic_orders",
        t=t,
        config_obj=config_obj,
    )

    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(f"Loading existing epic orders data from {output_filename}")
    ...
    if output_filename:
        ...
        logger.debug(f"Saving epic orders data to {output_filename}")
