...
from pat2vec.util.elasticsearch_index_config import CORE_O2_FIELDS
...
def search_core_o2_observations(
    ...
    output_filename: str | None = "core_o2_search_results.csv",
    ...
    ----
        ...
        output_filename (Optional[str]): The filename or path to a CSV file to
            load from or save to. Defaults to "core_o2_search_results.csv".
    ...
    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(f"Loading existing core O2 data from {output_filename}")
    ...
    if observations_time_field is None:
        msg = "observations_time_field cannot be None."
        raise ValueError(msg)
    if any(
        x is None
    ...
    if output_filename:
        ...
        logger.debug(f"Saving core O2 data to {output_filename}")
        results.to_csv(output_filename, index=False)

    return results


def clean_observation_value(value) -> str | None:
...
def get_core_02(
    ...
    if not config_obj.batch_mode and cohort_searcher_with_terms_and_search is None:
        msg = "cohort_searcher_with_terms_and_search cannot be None when not in batch mode."
        raise ValueError(
            msg,
        )
