...
from pat2vec.util.elasticsearch_index_config import EPIC_IMAGING_REPORTS_FIELDS
...
def search_epic_imaging_reports(
    ...
    output_filename: str | None = "epic_imaging_reports_results.csv",
    ...
    ----
        ...
        fields_override: Optional list of field names to include in the search
            results. If None, uses EPIC_IMAGING_REPORTS_FIELDS defaults.
        ...
        output_filename: Optional filename to save results as CSV. If provided
            ...
            Defaults to "epic_imaging_reports_results.csv".
    ...
    update_pbar(
        ...
        stage_int=0,
        stage_str="epic_imaging_reports",
        t=t,
    ...
    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(
            f"Loading existing epic imaging reports data from {output_filename}",
        )
    ...
    if output_filename:
        ...
        logger.debug(f"Saving epic imaging reports data to {output_filename}")
...
def get_epic_imaging_reports(
    ...
    ----
        current_pat_client_id_code: The unique identifier code for the patient
            to retrieve imaging reports data for.
        ...
        cohort_searcher_with_terms_and_search: Optional callable function for
            searching imaging reports when not in batch mode. Required when
            batch_mode=False.
    ...
    Returns:
    -------
        pd.DataFrame: DataFrame containing extracted features from imaging reports
    ...
    if "document_Name" in current_pat_raw.columns:
        ...
        for name_val in unique_names:
            ...
            features[f"epic_imaging_report_{sanitized_name}"] = 1
