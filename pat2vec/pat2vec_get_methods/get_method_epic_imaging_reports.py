...
def search_epic_imaging_reports(
    ...
    Uses a cohort searcher to query the Elasticsearch index for Epic imaging
    reports. If `output_filename` is provided, the function will attempt to load
    ...
    Args:
        ...
        index_name (str): The name of the Elasticsearch index to search.
            Defaults to "epic_imaging_reports".
        output_filename (Optional[str]): The filename or path to a CSV file to
            load from or save to. Defaults to "epic_imaging_reports_results.csv".
    ...
    Returns:
        pd.DataFrame: A DataFrame containing the raw Epic imaging reports data.
...
def get_epic_imaging_reports(
    ...
    """Retrieves Epic imaging reports features for a patient within a date range.

    This function retrieves epic imaging reports data, either from a pre-loaded
    batch DataFrame or by querying the database, and then processes it to create
    one-hot encoded binary features based on imaging modality and study status.
    ...
    Returns:
        pd.DataFrame: A DataFrame containing epic imaging report features for the
            specified patient. Binary columns are created for each unique modality and status.
