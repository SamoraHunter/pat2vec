...
from pat2vec.util.elasticsearch_index_config import SMOKING_FIELDS
...
def search_smoking(
    ...
    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(f"Loading existing smoking data from {output_filename}")
    ...
    if output_filename:
        ...
        logger.debug(f"Saving smoking data to {output_filename}")
