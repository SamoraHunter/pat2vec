...
from pat2vec.util.elasticsearch_index_config import EPIC_MEDICAL_HISTORY_FIELDS
...
def search_epic_medical_history(
    ...
    output_filename: str | None = "epic_medical_history_results.csv",
    ...
    ----
        ...
        fields_override: Optional list of field names to include in the search
            results. If None, uses EPIC_MEDICAL_HISTORY_FIELDS defaults.
        ...
        output_filename: Optional filename to save results as CSV. If provided
            ...
            Defaults to "epic_medical_history_results.csv".
    ...
    update_pbar(
        ...
        stage_int=0,
        stage_str="epic_medical_history",
        t=t,
    ...
    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(
            f"Loading existing epic medical history data from {output_filename}",
        )
    ...
    if output_filename:
        ...
        logger.debug(f"Saving epic medical history data to {output_filename}")
...
def get_epic_medical_history(
    ...
    ----
        current_pat_client_id_code: The unique identifier code for the patient
            to retrieve medical history data for.
        ...
        cohort_searcher_with_terms_and_search: Optional callable function for
            searching medical history when not in batch mode. Required when
            batch_mode=False.
    ...
    Returns:
    -------
        pd.DataFrame: DataFrame containing extracted features from medical history
            including binary flags for unique document names observed.
            Always includes 'client_idcode' column.
    ...
    if "document_Name" in current_pat_raw.columns:
        ...
        for name_val in unique_names:
            ...
            features[f"epic_medical_history_{sanitized_name}"] = 1
