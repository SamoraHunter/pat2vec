...
def search_core_resus_observations(
    ...
    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(f"Loading existing core resus data from {output_filename}")
    ...
    if output_filename:
        ...
        logger.debug(f"Saving core resus data to {output_filename}")
...
def get_core_resus(
    ...
    if config_obj is None:
        msg = "config_obj cannot be None. Please provide a valid configuration object."
        raise ValueError(
            msg,
        )
