...
from pat2vec.util.elasticsearch_index_config import EPIC_CLINICAL_NOTES_FIELDS
...
def search_epic_clinical_notes(
    ...
    output_filename: str | None = "epic_clinical_notes_results.csv",
    ...
    ----
        ...
        fields_override: Optional list of field names to include in the search
            results. If None, uses EPIC_CLINICAL_NOTES_FIELDS defaults.
        ...
        output_filename: Optional filename to save results as CSV. If provided
            ...
            Defaults to "epic_clinical_notes_results.csv".
    ...
    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(
            f"Loading existing epic clinical notes data from {output_filename}",
        )
    ...
    if output_filename:
        ...
        logger.debug(f"Saving epic clinical notes data to {output_filename}")
...
def get_epic_clinical_notes(
    ...
    ----
        current_pat_client_id_code: The unique identifier code for the patient
            to retrieve clinical notes data for.
        ...
        cohort_searcher_with_terms_and_search: Optional callable function for
            searching clinical notes when not in batch mode. Required when
            batch_mode=False.
    ...
    Returns:
    -------
        pd.DataFrame: DataFrame containing extracted features from clinical notes
    ...
    if "document_Name" in current_pat_raw.columns:
        ...
        for name_val in unique_names:
            ...
            features[f"epic_note_{sanitized_name}"] = 1
