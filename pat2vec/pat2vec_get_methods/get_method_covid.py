...
def search_covid(
    ...
    output_filename: str | None = "covid_search_results.csv",
    ...
    ----
        ...
        output_filename (Optional[str]): The filename or path to a CSV file to
            load from or save to. Defaults to "covid_search_results.csv".
    ...
    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(f"Loading existing covid data from {output_filename}")
    ...
    if output_filename:
        ...
        logger.debug(f"Saving covid data to {output_filename}")
