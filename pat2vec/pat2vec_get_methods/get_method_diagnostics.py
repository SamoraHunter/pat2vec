...
def search_diagnostic_orders(
    ...
    ----
        ...
        fields_override (Optional[List[str]]): A list of fields to override the
            default `DIAGNOSTICS_FIELDS`. Defaults to None.
        ...
        output_filename (Optional[str]): The filename or path to a CSV file to
            load from or save to. Defaults to "diagnostics_search_results.csv".
    ...
    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(f"Loading existing diagnostics data from {output_filename}")
    ...
    fields_to_use = DIAGNOSTICS_FIELDS
    if fields_override:
    ...
    if output_filename:
        ...
        logger.debug(f"Saving diagnostics data to {output_filename}")
...
def get_current_pat_diagnostics(
    ...
    # --- Integrate Epic Orders for Diagnostics if enabled --- #
    # This integration is for diagnostics, so we filter Epic orders
    # to only include types that might represent diagnostic orders.
    if config_obj.main_options.get("epic_orders", False):
        if config_obj.verbosity >= 1:
            logger.debug("Fetching Epic Orders for diagnostics.")
        ...
        epic_order_data = search_epic_orders(
            ...
            end_month=end_month,
            end_day=end_day,
            additional_custom_search_string='document_OrderClass:("Imaging" OR "Procedure" OR "Diagnostic")',  # Filter for diagnostic-like orders
        ...
        if not epic_order_data.empty:
            ...
            epic_order_data["order_typecode"] = "diagnostic"  # Explicitly set type

            # Concatenate Epic order data with existing diagnostics data
            diagnostics = pd.concat([diagnostics, epic_order_data], ignore_index=True)
