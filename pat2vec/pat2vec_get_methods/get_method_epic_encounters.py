...
from pat2vec.util.elasticsearch_index_config import EPIC_ENCOUNTER_FIELDS
...
def search_epic_encounters(
    ...
    output_filename: str | None = "epic_encounters_results.csv",
    ...
    update_pbar(
        ...
        stage_int=0,
        stage_str="epic_encounters",
        t=t,
        config_obj=config_obj,
    )

    if output_filename and os.path.exists(output_filename) and not overwrite:
        logger.debug(f"Loading existing epic encounters data from {output_filename}")
    ...
    if output_filename:
        ...
        logger.debug(f"Saving epic encounters data to {output_filename}")
