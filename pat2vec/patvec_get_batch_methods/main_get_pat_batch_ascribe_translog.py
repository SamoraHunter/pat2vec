...
def get_pat_batch_ascribe_translog(
    current_pat_client_id_code: str,
    search_term: str,
    config_obj: Any,
    cohort_searcher_with_terms_and_search: Any,
) -> pd.DataFrame:
    """Retrieves a batch of ascribe_translog data for a patient using nhsnumber.

    This function retrieves ascribe translog data in batch mode, prioritizing the
    nhsnumber field. Falls back to casenumber if needed.

    Args:
    ----
        current_pat_client_id_code: The patient's NHS number.
    ...
    Returns:
    -------
        A DataFrame containing the batch of ascribe_translog records.
    ...
    if config_obj.storage_backend == "database":
        try:
            table_name = "raw_ascribe_translog"
    ...
    batch_obs_target_path = os.path.join(
        config_obj.pre_ascribe_translog_batch_path,
    ...
    try:
        if should_fetch:
            batch_target = cohort_searcher_with_terms_and_search(
                index_name="ascribe_translog",
                fields_list=ASCRIBE_TRANSGLOG_FIELDS,
                term_name="nhsnumber",
                entered_list=[current_pat_client_id_code],
                search_string=f"logdatetime:[{global_start_year}-{global_start_month}-{global_start_day} TO {global_end_year}-{global_end_month}-{global_end_day}]",
            )
            # If no data found with nhsnumber, try casenumber for backward compatibility
            if batch_target.empty:
                batch_target = cohort_searcher_with_terms_and_search(
                    index_name="ascribe_translog",
                    fields_list=ASCRIBE_TRANSGLOG_FIELDS,
                    term_name="casenumber",
                    entered_list=[current_pat_client_id_code],
                    search_string=f"logdatetime:[{global_start_year}-{global_start_month}-{global_start_day} TO {global_end_year}-{global_end_month}-{global_end_day}]",
                )
                            ...
                            with engine.begin() as connection:
                                ...
                                if config_obj.overwrite_stored_pat_observations:
                                    ...
                                    if inspector.has_table(db_table, schema=db_schema):
                                        del_query = text(
                                            f"DELETE FROM {db_table if engine.name == 'sqlite' else f'{schema_name}.{table_name}'} WHERE nhsnumber = :pat_id",
                                        )
                                        connection.execute(
                                            del_query,
