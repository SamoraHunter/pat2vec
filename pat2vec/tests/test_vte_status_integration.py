...
class TestVTEStatusIntegration(unittest.TestCase):
    """
    Comprehensive integration test for VTE Status lifecycle:
    1. Synthetic data generation (VTE status records).
    ...
    def setUp(self):
        self.test_patient_id = "P_VTE_INTEGRATION_TEST"
        ...
        self.config.vte_status_time_field = (
    ...
    def test_vte_status_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw VTE Status data)
        raw_vte_df = generate_vte_status_data(
            num_rows=3,
        ...
        # VTE Status data typically uses a configured time field or 'updatetime'.
        time_field = getattr(self.config, "vte_status_time_field", "updatetime")
        raw_vte_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_vte_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_vte_status)
        save_raw_patient_batch(
            raw_vte_df, self.test_patient_id, "raw_vte_status", self.config
        )
        db_raw_vte = get_df_from_db(
            self.config,
            "raw_data",
            "raw_vte_status",
            patient_ids=[self.test_patient_id],
        )
        self.assertFalse(db_raw_vte.empty, "Raw VTE Status data should be in DB.")
        self.assertEqual(len(db_raw_vte), 3)
        ...
        features_inclusive = get_vte_status(
        ...
        features_exclusive = get_vte_status(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_vte,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No VTE Status features should have signal for exclusive date range.",
            )
