...
class TestTextualObsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Textual Observations lifecycle:
    1. Synthetic data generation (raw Textual Obs).
    ...
    def setUp(self):
        self.test_patient_id = "P_TEXT_OBS_ANN_TEST"
    ...
    def test_textual_obs_full_integration_lifecycle(self, mock_pipeline):
        # 0. Mock transformer pipeline to avoid external network calls and SSLErrors
        mock_gen = MagicMock()
        mock_gen.return_value = [
            {"generated_text": "Sample observation text mentioning Fever."}
        ]
        mock_pipeline.return_value = mock_gen

        # 1. Synthetic Data Generation (Raw Textual Observations)
        raw_textual_obs_df = generate_basic_observations_textual_obs_data(
            num_rows=3,
        ...
        # Textual observations typically use basicobs_entered
        raw_textual_obs_df["basicobs_entered"] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_textual_obs_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_textual_obs)
        save_raw_patient_batch(
            raw_textual_obs_df, self.test_patient_id, "raw_textual_obs", self.config
        )
        db_raw_textual_obs = get_df_from_db(
            self.config,
            "raw_data",
            "raw_textual_obs",
        ...
        self.assertFalse(
            db_raw_textual_obs.empty, "Raw Textual Obs data should be in DB."
        )
        self.assertEqual(len(db_raw_textual_obs), 3)

        # 3. Annotation processing and storage into DB (annotations_ann_textual_obs)
        ...
        annotated_df = multi_annots_to_df_textual_obs(
            self.test_patient_id,
            db_raw_textual_obs,
            multi_annots,
            self.config,
        ...
        self.assertFalse(
            db_ann_textual_obs.empty, "Annotated Textual Obs data should be in DB."
        )
