...
class TestEpicImagingReportsAnnotationsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Epic Imaging Reports Annotations lifecycle:
    1. Synthetic data generation (raw Epic Imaging Reports).
    ...
    def setUp(self):
        self.test_patient_id = "P_EPIC_IMG_ANN_TEST"
    ...
    def test_epic_imaging_reports_annotations_full_integration_lifecycle(
        self, mock_pipeline
    ):
        # 0. Mock transformer pipeline to avoid external network calls
        mock_gen = MagicMock()
        mock_gen.return_value = [
            {"generated_text": "Sample imaging report text mentioning Fever."}
        ]
        mock_pipeline.return_value = mock_gen

        # 1. Synthetic Data Generation (Raw Epic Imaging Reports)
        raw_img_df = generate_epic_imaging_reports_data(
            num_rows=3,
        ...
        raw_img_df["document_CreatedWhen"] = [  # Changed to match generator
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_img_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_epic_imaging_reports)
        save_raw_patient_batch(
            raw_img_df, self.test_patient_id, "raw_epic_imaging_reports", self.config
        )
        db_raw_img = get_df_from_db(
            self.config,
            "raw_data",
        ...
        self.assertFalse(
            db_raw_img.empty, "Raw Epic Imaging Reports data should be in DB."
        )

        # 3. Annotation processing and storage into DB (annotations_ann_epic_imaging_reports)
        ...
        annotated_df = multi_annots_to_df_epic_imaging_reports(
            self.test_patient_id, db_raw_img, multi_annots, self.config, mock_tqdm
        )
        # For SQLite, the schema name is prepended to the table name
        annot_table = "annotations_ann_epic_imaging_reports"
        annotated_df.to_sql(annot_table, self.engine, index=False, if_exists="replace")

        db_ann_img = get_df_from_db(
            self.config,
            "annotations",
        ...
        self.assertFalse(
            db_ann_img.empty, "Annotated Epic Imaging Reports data should be in DB."
        )
