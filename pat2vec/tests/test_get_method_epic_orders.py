...
def test_get_epic_orders():
    """Test the get_epic_orders function with dummy data."""
    config = MockConfig()
    patient_id = "P123456"
    ...
    # Verify that binary features are extracted from orders (class and status)
    assert any(col.startswith("epic_order_") for col in df.columns) or df.empty
