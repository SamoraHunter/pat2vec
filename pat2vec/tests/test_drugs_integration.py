...
from pat2vec.util.get_dummy_data_cohort_searcher import generate_drugs_data
...
class TestDrugsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Drugs (Medications) lifecycle:
    1. Synthetic data generation (medication orders).
    ...
    def setUp(self):
        self.test_patient_id = "P_DRUGS_INTEGRATION_TEST"
        ...
        self.config.drug_time_field = "order_createdwhen"  # Align with dummy data
        self.engine = self.config.db_engine

    def tearDown(self):
        self.engine.dispose()

    def test_drugs_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Drugs/Orders)
        raw_drugs_df = generate_drugs_data(
            num_rows=3,
        ...
        # Medications typically use 'order_createdwhen' for temporal filtering.
        time_field = self.config.drug_time_field
        raw_drugs_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_drugs_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_drugs)
        save_raw_patient_batch(
            raw_drugs_df, self.test_patient_id, "raw_drugs", self.config
        )
        db_raw_drugs = get_df_from_db(
            self.config, "raw_data", "raw_drugs", patient_ids=[self.test_patient_id]
        )
        self.assertFalse(db_raw_drugs.empty, "Raw Drugs data should be in DB.")
        self.assertEqual(len(db_raw_drugs), 3)
        ...
        features_inclusive = get_current_pat_drugs(
        ...
        features_exclusive = get_current_pat_drugs(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_drugs,
            config_obj=self.config,
        ...
        # For drugs, if no rows match the temporal filter, numeric features should be absent, zero, or NaN
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No drug features should have signal for exclusive date range.",
            )
