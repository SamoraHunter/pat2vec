...
class TestEpicOrdersAnnotationsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Epic Orders Annotations lifecycle:
    1. Synthetic data generation (raw Epic Orders).
    ...
    def setUp(self):
        self.test_patient_id = "P_EPIC_ORDER_ANN_TEST"
        ...
        self.config = config_class(
            ...
            batch_mode=True,
            main_options={
                "epic_orders": True,  # Enable raw epic orders
                "epic_orders_annotations": True,  # Enable annotation processing for epic orders
            },
        )
    ...
    def test_epic_orders_annotations_full_integration_lifecycle(self, mock_pipeline):
        # 0. Mock transformer pipeline to avoid external network calls and SSLErrors
        mock_gen = MagicMock()
        mock_gen.return_value = [
            {"generated_text": "Sample clinical text mentioning Fever."}
        ]
        mock_pipeline.return_value = mock_gen

        # 1. Synthetic Data Generation (Raw Epic Orders)
        raw_epic_orders_df = generate_epic_orders_data(
            num_rows=3,
        ...
        raw_epic_orders_df["document_CreatedWhen"] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_epic_orders_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_epic_orders)
        save_raw_patient_batch(
            raw_epic_orders_df,
        ...
        db_raw_epic_orders = get_df_from_db(
            self.config,
            "raw_data",
            "raw_epic_orders",
            patient_ids=[self.test_patient_id],
            patient_id_column="document_PatientDurableKey",
        )
        self.assertFalse(
            db_raw_epic_orders.empty, "Raw Epic Orders data should be in DB."
        )
        self.assertEqual(len(db_raw_epic_orders), 3)

        # 3. Annotation processing and storage of annotations into DB (ann_epic_orders)
        mock_tqdm = MagicMock(spec=tqdm.tqdm)
        cat = get_cat(self.config)  # Use getter to exercise the patch
        ...
        annotated_df = multi_annots_to_df_epic_orders(
            self.test_patient_id,
            db_raw_epic_orders,
            multi_annots,
            self.config,
        ...
        self.assertFalse(
            db_ann_epic_orders.empty, "Annotated Epic Orders data should be in DB."
        )
        self.assertGreater(
            len(db_ann_epic_orders), 0
        )  # Dummy annotations should generate some entries

        # 4. Feature Vectorization & Storage (Correct Date Range)
        features_inclusive = get_current_pat_epic_orders_annotations(
            self.test_patient_id,
            self.target_date_range_inclusive,
            db_ann_epic_orders,
        ...
        # Assert generic feature existence rather than specific entity names
        self.assertTrue(
            any(
        ...
        features_exclusive = get_current_pat_epic_orders_annotations(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_ann_epic_orders,
            config_obj=self.config,
