...
class TestEpicMedicalHistoryAnnotationsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Epic Medical History Annotations lifecycle:
    1. Synthetic data generation (raw Epic Medical History).
    ...
    def setUp(self):
        self.test_patient_id = "P_EPIC_MED_HIST_ANN_TEST"
    ...
    def test_epic_medical_history_annotations_full_integration_lifecycle(
        self, mock_pipeline
    ):
        # 0. Mock transformer pipeline to avoid external network calls
        mock_gen = MagicMock()
        mock_gen.return_value = [
            {"generated_text": "Sample medical history text mentioning Fever."}
        ]
        mock_pipeline.return_value = mock_gen

        # 1. Synthetic Data Generation (Raw Epic Medical History)
        raw_med_hist_df = generate_epic_medical_history_data(
            num_rows=3,
        ...
        raw_med_hist_df["document_CreatedWhen"] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_med_hist_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_epic_medical_history)
        ...
        db_raw_med_hist = get_df_from_db(
            self.config,
            "raw_data",
            "raw_epic_medical_history",
        ...
        self.assertFalse(
            db_raw_med_hist.empty, "Raw Epic Medical History data should be in DB."
        )

        # 3. Annotation processing and storage into DB (annotations_ann_epic_medical_history)
        ...
        multi_annots = annot_pat_batch_docs(
            ...
            self.config,
            mock_tqdm,
            text_column="document_Comment",  # Changed to match generator
        )
        annotated_df = multi_annots_to_df_epic_medical_history(
            self.test_patient_id, db_raw_med_hist, multi_annots, self.config, mock_tqdm
        )
        # For SQLite, the schema name is prepended to the table name
        annot_table = "annotations_ann_epic_medical_history"
        annotated_df.to_sql(annot_table, self.engine, index=False, if_exists="replace")

        db_ann_med_hist = get_df_from_db(
            self.config,
            "annotations",
        ...
        self.assertFalse(
            db_ann_med_hist.empty,
            "Annotated Epic Medical History data should be in DB.",
        )
