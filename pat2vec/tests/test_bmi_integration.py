...
class TestBMIIntegration(unittest.TestCase):
    """
    Comprehensive integration test for BMI (Body Mass Index) lifecycle:
    1. Synthetic data generation (BMI records).
    ...
    def setUp(self):
        self.test_patient_id = "P_BMI_INTEGRATION_TEST"
    ...
    def test_bmi_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw BMI data)
        ...
        # BMI data typically uses a configured time field or 'updatetime'.
        time_field = getattr(self.config, "bmi_time_field", "updatetime")
        raw_bmi_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_bmi_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_bmi)
        save_raw_patient_batch(raw_bmi_df, self.test_patient_id, "raw_bmi", self.config)
        db_raw_bmi = get_df_from_db(
            self.config, "raw_data", "raw_bmi", patient_ids=[self.test_patient_id]
        )
        self.assertFalse(db_raw_bmi.empty, "Raw BMI data should be in DB.")
        self.assertEqual(len(db_raw_bmi), 3)
        ...
        features_inclusive = get_bmi_features(
        ...
        features_exclusive = get_bmi_features(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_bmi,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No BMI features should have signal for exclusive date range.",
            )
