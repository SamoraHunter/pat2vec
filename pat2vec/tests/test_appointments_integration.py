...
from pat2vec.util.get_dummy_data_cohort_searcher import generate_appointments_data
...
class TestAppointmentsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Appointments lifecycle:
    1. Synthetic data generation (outpatient/inpatient appointments).
    ...
    def setUp(self):
        self.test_patient_id = "P_APP_INTEGRATION_TEST"
        ...
        self.config.appointments_time_field = (
            "AppointmentDateTime"  # Align with dummy data
        )
    ...
    def test_appointments_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Appointments)
        raw_app_df = generate_appointments_data(
            num_rows=3,
        ...
        # Appointments typically use a configured time field or fallback to 'updatetime'.
        time_field = getattr(self.config, "appointments_time_field", "updatetime")
        raw_app_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_app_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_appointments).
        # Note: Appointments use 'HospitalID' as the identifier in the raw index.
        # We explicitly set id_column to match this and avoid the helper adding
        # a default 'client_idcode' which would conflict with downstream mapping.
        save_raw_patient_batch(
            raw_app_df,
        ...
        self.assertFalse(db_raw_app.empty, "Raw Appointments data should be in DB.")
        self.assertEqual(len(db_raw_app), 3)
        ...
        features_inclusive = get_appointments(
            self.test_patient_id,
            self.target_date_range_inclusive,
            db_raw_app,
        ...
        features_exclusive = get_appointments(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_app,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No appointment features should have signal for exclusive date range.",
            )
