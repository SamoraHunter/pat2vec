...
class TestEpicClinicalNotesIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Epic Clinical Notes lifecycle:
    1. Synthetic data generation (raw Epic Clinical Notes).
    ...
    def setUp(self):
        self.test_patient_id = "P_EPIC_NOTES_TEST"
        ...
        self.config.epic_clinical_notes_time_field = (
    ...
    def test_epic_clinical_notes_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Epic Clinical Notes)
        ...
        # Epic clinical notes typically use 'document_CreatedWhen' for temporal filtering.
        time_field = "document_CreatedWhen"
        raw_notes_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_notes_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_epic_clinical_notes)
        save_raw_patient_batch(
            raw_notes_df, self.test_patient_id, "raw_epic_clinical_notes", self.config
        )
        ...
        self.assertFalse(
            db_raw_notes.empty, "Raw Epic Clinical Notes data should be in DB."
        )
        self.assertEqual(len(db_raw_notes), 3)

        # 3. Feature Vectorization & Storage (Correct Date Range)
        # Using positional arguments for (ID, DateRange, Batch) for consistency
        features_inclusive = get_epic_clinical_notes(
        ...
        self.assertTrue(
            any(
                col.startswith("epic_note_type_") for col in features_inclusive.columns
            ),
            "Epic Note Type feature columns should be present.",
        )
        ...
        features_exclusive = get_epic_clinical_notes(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_notes,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No Epic Note features should have signal for exclusive date range.",
            )
