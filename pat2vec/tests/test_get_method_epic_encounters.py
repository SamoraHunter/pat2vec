...
def test_get_epic_encounters():
    """Test the get_epic_encounters function with dummy data."""
    config = MockConfig()
    patient_id = "P123456"
    ...
    # Verify that at least some epic features are extracted
    assert any(col.startswith("epic_enc_") for col in df.columns) or df.empty
