...
class TestEpicClinicalNotesAppointmentsIntegration(unittest.TestCase):
    ...
    def setUp(self):
        self.test_patient_id = "P_EPIC_NOTES_APP_TEST"
    ...
    def test_epic_clinical_notes_appointments_full_integration_lifecycle(
        ...
        self.assertFalse(
            ...
            "Raw Epic Clinical Note Appointments data should be in DB.",
        )
        self.assertEqual(len(db_raw_notes_app), 3)

        # 3. Feature Vectorization & Storage (Correct Date Range)
        # Using positional arguments for (ID, DateRange, Batch) for consistency
        features_inclusive = get_epic_clinical_notes_appointments(
            self.test_patient_id,
            self.target_date_range_inclusive,
            db_raw_notes_app,
        ...
        self.assertTrue(
            any(
                col.startswith("epic_note_appt_name_")
                for col in features_inclusive.columns
            ),
            "Epic Note Appointment Name feature columns should be present.",
        )
        ...
        features_exclusive = get_epic_clinical_notes_appointments(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_notes_app,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No Epic Note Appointment features should have signal for exclusive date range.",
            )
