...
class TestEpicEncountersIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Epic Encounters lifecycle:
    1. Synthetic data generation (raw Epic Encounters).
    ...
    def setUp(self):
        self.test_patient_id = "P_EPIC_ENC_TEST"
    ...
    def test_epic_encounters_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Epic Encounters)
        raw_enc_df = generate_epic_encounters_data(
            num_rows=3,
        ...
        # Epic encounters typically use 'activity_AdmissionDate' for temporal filtering.
        time_field = "activity_AdmissionDate"
        raw_enc_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_enc_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_epic_encounters)
        save_raw_patient_batch(
            raw_enc_df, self.test_patient_id, "raw_epic_encounters", self.config
        )
        db_raw_enc = get_df_from_db(
            self.config,
            "raw_data",
            "raw_epic_encounters",
            patient_ids=[self.test_patient_id],
        )
        self.assertFalse(db_raw_enc.empty, "Raw Epic Encounters data should be in DB.")
        self.assertEqual(len(db_raw_enc), 3)
        ...
        features_inclusive = get_epic_encounters(
        ...
        self.assertTrue(
            any(col.startswith("epic_enc_") for col in features_inclusive.columns),
            "Epic Enc feature columns should be present.",
        )
        ...
        features_exclusive = get_epic_encounters(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_enc,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No Epic Enc features should have signal for exclusive date range.",
            )
