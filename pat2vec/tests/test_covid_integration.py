...
class TestCOVIDIntegration(unittest.TestCase):
    """
    Comprehensive integration test for COVID Status lifecycle:
    1. Synthetic data generation (COVID-19 status records).
    ...
    def setUp(self):
        self.test_patient_id = "P_COVID_INTEGRATION_TEST"
        ...
        self.config.covid_time_field = "basicobs_entered"  # Align with dummy data
        self.engine = self.config.db_engine

    def tearDown(self):
        self.engine.dispose()

    def test_covid_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw COVID Status data)
        raw_covid_df = generate_covid_data(
            num_rows=3,
        ...
        # COVID Status data typically uses a configured time field or 'updatetime'.
        time_field = getattr(self.config, "covid_time_field", "updatetime")
        raw_covid_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_covid_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_covid)
        save_raw_patient_batch(
            raw_covid_df, self.test_patient_id, "raw_covid", self.config
        )
        db_raw_covid = get_df_from_db(
            self.config, "raw_data", "raw_covid", patient_ids=[self.test_patient_id]
        )
        self.assertFalse(db_raw_covid.empty, "Raw COVID data should be in DB.")
        self.assertEqual(len(db_raw_covid), 3)
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No COVID features should have signal for exclusive date range.",
            )
