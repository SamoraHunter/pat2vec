...
def test_get_epic_clinical_notes():
    """Test the get_epic_clinical_notes function with dummy data."""
    config = MockConfig()
    patient_id = "P123456"
    ...
    # Verify that binary features are extracted from note names
    assert any(col.startswith("epic_note_type_") for col in df.columns) or df.empty
