...
from pat2vec.util.get_dummy_data_cohort_searcher import generate_reports_data, dummy_CAT
...
class TestReportsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Reports Annotations lifecycle:
    1. Synthetic data generation (raw Reports).
    ...
    def setUp(self):
        self.test_patient_id = "P_REPORTS_ANN_TEST"
        ...
        self.config = config_class(
            ...
            main_options={
                "annotations_reports": True,  # Enable annotation processing for Reports
            },
            days=0,
        )
        self.config.reports_time_field = "updatetime"  # Align with dummy data
    ...
    def test_reports_full_integration_lifecycle(self, mock_pipeline):
        # 0. Mock transformer pipeline to avoid external network calls and SSLErrors
        mock_gen = MagicMock()
        mock_gen.return_value = [
            {"generated_text": "Sample report text mentioning Fever."}
        ]
        mock_pipeline.return_value = mock_gen

        # 1. Synthetic Data Generation (Raw Reports)
        raw_reports_df = generate_reports_data(
            num_rows=3,
        ...
        # Reports typically use 'updatetime' for temporal filtering
        raw_reports_df["updatetime"] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_reports_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_reports)
        save_raw_patient_batch(
            raw_reports_df, self.test_patient_id, "raw_reports", self.config
        )
        db_raw_reports = get_df_from_db(
            self.config, "raw_data", "raw_reports", patient_ids=[self.test_patient_id]
        )
        self.assertFalse(db_raw_reports.empty, "Raw Reports data should be in DB.")
        self.assertEqual(len(db_raw_reports), 3)

        # 3. Annotation processing and storage into DB (annotations_ann_reports)
        ...
        multi_annots = annot_pat_batch_docs(
            ...
            self.config,
            mock_tqdm,
            text_column="textualObs",  # Reports use textualObs for the text body
        )
        ...
        self.assertFalse(
            db_ann_reports.empty, "Annotated Reports data should be in DB."
        )
        self.assertGreater(len(db_ann_reports), 0)
