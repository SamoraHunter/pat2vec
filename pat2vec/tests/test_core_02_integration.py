...
from pat2vec.util.get_dummy_data_cohort_searcher import generate_core_02_data
...
class TestCore02Integration(unittest.TestCase):
    """
    Comprehensive integration test for Core O2 lifecycle:
    1. Synthetic data generation (Oxygen saturation records).
    ...
    def setUp(self):
        self.test_patient_id = "P_CORE02_INTEGRATION_TEST"
        ...
        self.config.core_02_time_field = (
    ...
    def test_core_02_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Core O2 data)
        raw_core_02_df = generate_core_02_data(
            num_rows=3,
        ...
        # Core O2 data typically uses a configured time field or 'updatetime'.
        time_field = getattr(self.config, "core_02_time_field", "updatetime")
        raw_core_02_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_core_02_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_core_02)
        save_raw_patient_batch(
            raw_core_02_df, self.test_patient_id, "raw_core_02", self.config
        )
        db_raw_core_02 = get_df_from_db(
            self.config, "raw_data", "raw_core_02", patient_ids=[self.test_patient_id]
        )
        self.assertFalse(db_raw_core_02.empty, "Raw Core O2 data should be in DB.")
        self.assertEqual(len(db_raw_core_02), 3)
        ...
        features_inclusive = get_core_02(
        ...
        features_exclusive = get_core_02(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_core_02,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No Core O2 features should have signal for exclusive date range.",
            )
