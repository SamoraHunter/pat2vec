...
from pat2vec.util.methods_annotation import annot_pat_batch_docs, multi_annots_to_df_epr_docs  # type: ignore
...
class TestEPRDocsAnnotationsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for EPR Documents Annotations lifecycle:
    1. Synthetic data generation (raw EPR Docs).
    ...
    def setUp(self):
        self.test_patient_id = "P_EPR_DOC_ANN_TEST"
        ...
        self.config = config_class(
            ...
            batch_mode=True,
            main_options={
                "annotations": True,  # Enable annotation processing for EPR documents
            },
        )
    ...
    def test_epr_docs_annotations_full_integration_lifecycle(self, mock_pipeline):
        ...
        raw_epr_docs_df["updatetime"] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_epr_docs_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_epr_docs)
        save_raw_patient_batch(
            raw_epr_docs_df, self.test_patient_id, "raw_epr_docs", self.config
        )
        db_raw_epr_docs = get_df_from_db(
            self.config, "raw_data", "raw_epr_docs", patient_ids=[self.test_patient_id]
        )
        self.assertFalse(db_raw_epr_docs.empty, "Raw EPR Docs data should be in DB.")
        self.assertEqual(len(db_raw_epr_docs), 3)

        # 3. Annotation processing and storage into DB (annotations_ann_epr_docs)
        ...
        annotated_df = multi_annots_to_df_epr_docs(
            self.test_patient_id,
            db_raw_epr_docs,
            multi_annots,
            self.config,
        ...
        self.assertFalse(
            db_ann_epr_docs.empty, "Annotated EPR Docs data should be in DB."
        )
        self.assertGreater(len(db_ann_epr_docs), 0)
