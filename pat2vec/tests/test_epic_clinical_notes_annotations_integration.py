...
class TestEpicClinicalNotesAnnotationsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Epic Clinical Notes Annotations lifecycle:
    ...
    def test_epic_clinical_notes_annotations_full_integration_lifecycle(
        ...
        # 3. Annotation processing and storage into DB (annotations_ann_epic_clinical_notes)
        ...
        annotated_df = multi_annots_to_df_epic_clinical_notes(
            self.test_patient_id, db_raw_notes, multi_annots, self.config, mock_tqdm
        )
        ...
        self.assertFalse(
            db_ann_notes.empty, "Annotated Epic Clinical Notes data should be in DB."
        )
