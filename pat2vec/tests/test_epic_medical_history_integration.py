...
class TestEpicMedicalHistoryIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Epic Medical History lifecycle:
    ...
    def setUp(self):
        self.test_patient_id = "P_EPIC_MED_HIST_TEST"
        ...
        self.config.epic_medical_history_time_field = (
    ...
    def test_epic_medical_history_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Epic Medical History)
        ...
        # Epic medical history typically uses 'document_CreatedWhen' for temporal filtering.
        time_field = "document_CreatedWhen"
        raw_med_hist_df[time_field] = [
        ...
        self.assertEqual(len(db_raw_med_hist), 3)
        ...
        features_inclusive = get_epic_medical_history(
            self.test_patient_id,
            self.target_date_range_inclusive,
            db_raw_med_hist,
        ...
        self.assertTrue(
            any(col.startswith("epic_med_hist_") for col in features_inclusive.columns),
            "Epic Medical History feature columns should be present.",
        )
        ...
        features_exclusive = get_epic_medical_history(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_med_hist,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No Epic Medical History features should have signal for exclusive date range.",
            )
