...
from pat2vec.util.get_dummy_data_cohort_searcher import generate_diagnostics_data
...
class TestDiagnosticsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Diagnostics lifecycle:
    1. Synthetic data generation (diagnostic orders).
    ...
    def setUp(self):
        self.test_patient_id = "P_DIAG_INTEGRATION_TEST"
        ...
        self.config.diagnostic_time_field = "order_createdwhen"  # Align with dummy data
        self.engine = self.config.db_engine

    def tearDown(self):
        self.engine.dispose()

    def test_diagnostics_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Diagnostics/Orders)
        raw_diag_df = generate_diagnostics_data(
            num_rows=3,
        ...
        # Diagnostics typically use 'order_createdwhen' for temporal filtering in pat2vec.
        time_field = self.config.diagnostic_time_field
        raw_diag_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_diag_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_diagnostics)
        save_raw_patient_batch(
            raw_diag_df, self.test_patient_id, "raw_diagnostics", self.config
        )
        db_raw_diag = get_df_from_db(
            self.config,
            "raw_data",
            "raw_diagnostics",
            patient_ids=[self.test_patient_id],
        )
        self.assertFalse(db_raw_diag.empty, "Raw Diagnostics data should be in DB.")
        self.assertEqual(len(db_raw_diag), 3)
        ...
        features_inclusive = get_current_pat_diagnostics(
        ...
        features_exclusive = get_current_pat_diagnostics(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_diag,
            config_obj=self.config,
        ...
        # For diagnostics, if no rows match the temporal filter, numeric features should be absent, zero, or NaN
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No diagnostics features should have signal for exclusive date range.",
            )
