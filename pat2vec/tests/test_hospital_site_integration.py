...
class TestHospitalSiteIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Hospital Site lifecycle:
    1. Synthetic data generation (Hospital Site records).
    ...
    def setUp(self):
        self.test_patient_id = "P_HOSP_SITE_INTEGRATION_TEST"
        ...
        self.config.hosp_site_time_field = (
    ...
    def test_hospital_site_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Hospital Site data)
        # Hospital Site data is typically stored in the observations index.
        raw_hosp_site_df = generate_observations_data(
            num_rows=3,
            entered_list=[self.test_patient_id],
            search_term="Hospital Site",
        ...
        # Hospital Site data typically uses a configured time field or 'updatetime'.
        time_field = getattr(self.config, "hosp_site_time_field", "updatetime")
        raw_hosp_site_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_hosp_site_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_hospsite)
        save_raw_patient_batch(
            raw_hosp_site_df, self.test_patient_id, "raw_hospsite", self.config
        )
        db_raw_hosp_site = get_df_from_db(
            self.config, "raw_data", "raw_hospsite", patient_ids=[self.test_patient_id]
        )
        self.assertFalse(
            db_raw_hosp_site.empty, "Raw Hospital Site data should be in DB."
        )
        self.assertEqual(len(db_raw_hosp_site), 3)
        ...
        features_inclusive = get_hosp_site(
        ...
        features_exclusive = get_hosp_site(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_hosp_site,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No Hospital Site features should have signal for exclusive date range.",
            )
