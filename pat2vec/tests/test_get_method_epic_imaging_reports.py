...
def test_get_epic_imaging_reports():
    """Test the get_epic_imaging_reports function with dummy data."""
    config = MockConfig()
    patient_id = "P123456"
    ...
    # Verify that binary features are extracted from imaging reports
    assert any(col.startswith("epic_img_") for col in df.columns)
