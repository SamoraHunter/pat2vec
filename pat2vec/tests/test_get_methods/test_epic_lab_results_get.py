...
class TestEpicLabResultsGet:
    ...
    def _start_elastic(self, elastic_container):
        ...
        for pid in cls.patient_ids:
            ...
            lab_results_dfs.append(df)

        df_lab_results = (
            pd.concat(lab_results_dfs, ignore_index=True)
            if len(lab_results_dfs) > 1
            else lab_results_dfs[0]
        )
        df_lab_results = df_lab_results.where(pd.notnull(df_lab_results), None)
        ...
        cls.cs.elastic.indices.refresh(index="epic_lab_results")
        ...
        cls.config_obj = config_class(
            ...
            credentials_path=cls.cred_path,
            current_path_dir="",
            main_options={"epic_lab_results": True},
            batch_mode=True,
            verbosity=0,
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("epic_lab_results", False) is True
        ), "Epic lab results option should be enabled in config"
    ...
    def test_3_index_population_and_verification(self):
        ...
        es_count = self.cs.elastic.count(index="epic_lab_results")["count"]
        expected_lab_result_count = len(self.patient_ids) * 3
        assert (
            es_count >= expected_lab_result_count
        ), f"Expected at least {expected_lab_result_count} epic_lab_results, got {es_count}"
    ...
    def test_epic_lab_results_data_retrieval(self):
        ...
        lab_result_data = get_epic_lab_results(
        ...
        assert lab_result_data is not None, "Lab result data should not be None"
        assert isinstance(
            lab_result_data,
            pd.DataFrame,
        ), "Lab result data should be a DataFrame"
        assert not lab_result_data.empty, "Lab result DataFrame should not be empty"
        assert (
            "client_idcode" in lab_result_data.columns
        ), "Should have client_idcode column"

    def test_merge_epic_lab_results_data_functionality(self):
        """Test merge epic_lab_results data functionality - verify merge function creates CSV."""
        ...
        ), "Merged epic_lab_results DataFrame should not be empty"

    def test_6_cleanup_verification(self):
        ...
        merged_csv_path = os.path.join("merged_batches", "merged_epic_lab_results.csv")
        try:
            if os.path.exists(merged_csv_path):
                os.remove(merged_csv_path)
        except Exception as e:
            msg = f"Failed to remove merged CSV '{merged_csv_path}': {e}"
            raise AssertionError(msg) from e
        ...
        assert not os.path.exists(
            merged_csv_path,
        ), "Merged CSV file should be removed"
