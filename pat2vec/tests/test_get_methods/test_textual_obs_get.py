...
class TestTextualObsGet:
    """Stage-mirroring pytest for test_textual_obs_get.ipynb."""
    ...
    def _start_elastic(self, elastic_container):
        ...
        cls.DB_FILENAME = "temp_textual_obs_db.sqlite"
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        ), "Textual_obs option should be enabled in config"
    ...
    def test_textual_obs_vector_validation(self):
        ...
        feature_cols = [
            ...
            if "textual_obs" in c.lower() and c != "client_idcode"
        ]

        assert len(feature_cols) > 0, (
            f"No textual_obs-related columns found in feature vector. "
        ...
        assert len(totally_empty_cols) < len(feature_cols), (
            f"All textual_obs columns are empty - vectorisation is failing. "
            f"Null columns: {list(totally_empty_cols.index)}"
        )

        print(f"Found {len(feature_cols)} textual_obs feature columns")

    def test_textual_obs_data_retrieval(self):
        """Test textual_obs data retrieval - verify textual_obs features can be retrieved."""
        all_pat_list = self.pat2vec_obj.all_patient_list
        assert len(all_pat_list) > 0, "Patient list should not be empty"

        # Get all features (textual_obs data is stored in the features table)
        data_retrieved = get_all_features(self.config_obj)

        assert data_retrieved is not None, "Textual_obs data should not be None"
        if isinstance(data_retrieved, list):
            assert len(data_retrieved) > 0, "Textual_obs data list should not be empty"
            assert not data_retrieved[
                0
            ].empty, "Textual_obs DataFrame should not be empty"
        else:
