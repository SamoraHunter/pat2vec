...
class TestHospSiteGet:
    """Stage-mirroring pytest for test_hosp_site_get.ipynb."""
    ...
    def setup_class(cls: type) -> None:
        ...
        cls.PROJ_NAME = "hosp_site_test_project"
        cls.DB_FILENAME = "temp_hosp_site_db.sqlite"
        ...
        for dir_to_remove in ["hosp_site_test_project"]:
            try:
...
"""
        ...
        config_populate = config_class(
            proj_name="hosp_site_test_project",
        ...
        # Generate and ingest hosp_site data
        ...
        for pid in cls.patient_ids:
            ...
            hosp_site_dfs.append(df)

        df_hosp_site = (
            pd.concat(hosp_site_dfs, ignore_index=True)
            if len(hosp_site_dfs) > 1
            else hosp_site_dfs[0]
        )
    ...
    def test_2_config_and_pipeline_setup(self) -> None:
        ...
        assert (
            self.config_obj.main_options.get("hosp_site", False) is True
        ), "Hosp_site option should be enabled in config"
        assert self.pat2vec_obj is not None, "pat2vec object should not be None"

    def test_3_index_population_and_verification(self) -> None:
        ...
        # Verify hosp_site observations were ingested
        es_count = self.cs.elastic.count(index="observations")["count"]
        expected_hosp_site_count = len(self.patient_ids) * 3  # 5 patients * 3 rows each
        assert (
            es_count >= expected_hosp_site_count
        ), f"Expected at least {expected_hosp_site_count} hosp_site observations, got {es_count}"
    ...
    def test_hosp_site_data_retrieval(self) -> None:
        ...
        assert hosp_site_data is not None, "hosp_site data should not be None"
        if isinstance(hosp_site_data, list):
            assert len(hosp_site_data) > 0, "hosp_site data list should not be empty"
            assert not hosp_site_data[
                0
            ].empty, "hosp_site DataFrame should not be empty"
        else:
            assert not hosp_site_data.empty, "hosp_site DataFrame should not be empty"
    ...
    def test_merge_hosp_site_data_functionality(self) -> None:
        ...
        assert not merged_data.empty, "Merged hosp_site DataFrame should not be empty"
