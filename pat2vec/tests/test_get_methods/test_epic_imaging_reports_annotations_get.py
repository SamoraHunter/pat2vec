...
class TestEpicImagingReportsAnnotationsGet:
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("epic_imaging_reports_annotations", False)
            is True
        ), "Epic imaging reports annotations option should be enabled in config"
    ...
    def test_3_index_population_and_verification(self):
        ...
        # Verify epic_imaging_reports index has documents
        es_count = self.cs.elastic.count(index="epic_imaging_reports")["count"]
        expected_ir_count = len(self.patient_ids) * 3  # 5 patients * 3 rows each
        assert (
            es_count >= expected_ir_count
        ), f"Expected at least {expected_ir_count} imaging reports, got {es_count}"
    ...
    def test_epic_imaging_reports_annotations_data_retrieval(self):
        ...
        # Test the get method
        features_data = get_current_pat_epic_imaging_reports_annotations(
            current_pat_client_id_code=all_pat_list[0],
            target_date_range=(2020, 1, 1, 2023, 12, 31),
            epic_imaging_reports_annotations=annotations_data,
