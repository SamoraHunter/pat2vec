...
"""
        ...
        ingest_data_to_elasticsearch(
            df_encounters, "epic_encounters", es_client=cls.cs.elastic
        )
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("epic_encounters", False) is True
        ), "Epic encounters option should be enabled in config"
    ...
    def test_epic_encounters_data_retrieval(self):
        """Test epic encounters data retrieval - verify features can be retrieved."""
        ...
        assert encounter_data is not None, "Encounter data should not be None"
        if isinstance(encounter_data, list):
            assert len(encounter_data) > 0, "Encounter data list should not be empty"
            assert not encounter_data[
                0
            ].empty, "Encounter DataFrame should not be empty"
        else:
            assert not encounter_data.empty, "Encounter DataFrame should not be empty"
    ...
    def test_merge_epic_encounters_data_functionality(self):
        ...
        assert isinstance(merged_df, pd.DataFrame), "Merged data should be a DataFrame"
        assert (
            not merged_df.empty
        ), "Merged epic_encounters DataFrame should not be empty"
...
def merge_epic_encounters_data(config_obj):
    """Merge all epic_encounters data and raise ValueError if empty.

    Args:
        config_obj: Configuration object with database connection info

    Returns:
        pd.DataFrame: Merged epic_encounters feature data

    Raises:
        ValueError: If no epic_encounters data was extracted or returned
    ...
    if merged_df.empty:
        raise ValueError(
            "merge_epic_encounters_data() returned empty DataFrame — no data found in database"
        )
