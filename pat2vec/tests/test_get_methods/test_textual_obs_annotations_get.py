...
class TestTextualObsAnnotationsGet:
    """Stage-mirroring pytest for test_textual_obs_annotations_get.ipynb."""
    ...
    def _start_elastic(self, elastic_container):
        ...
        for pid in cls.patient_ids:
            ...
            textual_obs_dfs.append(df)

        df_textual_obs = (
            pd.concat(textual_obs_dfs, ignore_index=True)
            if len(textual_obs_dfs) > 1
            else textual_obs_dfs[0]
        )
        df_textual_obs = df_textual_obs.where(pd.notnull(df_textual_obs), None)
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("textual_obs", False) is True
        ), "Textual obs option should be enabled in config"
    ...
    def test_3_index_population_and_verification(self):
        ...
        # Verify textual_obs observations were ingested
        es_count = self.cs.elastic.count(index="basic_observations")["count"]
        expected_textual_obs_count = (
            len(self.patient_ids) * 3
        )  # 5 patients * 3 rows each
        assert (
            es_count >= expected_textual_obs_count
        ), f"Expected at least {expected_textual_obs_count} textual_obs observations, got {es_count}"
    ...
    def test_textual_obs_annotations_data_retrieval(self):
        """Test textual obs annotations data retrieval - verify annotations can be retrieved."""
        all_pat_list = self.pat2vec_obj.all_patient_list
        assert len(all_pat_list) > 0, "Patient list should not be empty"

        # Get textual obs annotations from database
        from pat2vec.util.helper_functions import get_df_from_db

        textual_obs_annotations = get_df_from_db(
            self.config_obj,
            "annotations",
            "ann_textual_obs",
        ...
        annotations_data = get_current_pat_textual_obs_annotations(
            current_pat_client_id_code=all_pat_list[0],
            target_date_range=target_date_range,
            textual_obs_annotations=textual_obs_annotations,
        ...
        ), "Textual obs annotations data should not be None"

    def test_textual_obs_annotations_count_features(self):
        """Test textual obs annotations count features - verify pretty_name counts are calculated."""
        from pat2vec.util.helper_functions import get_df_from_db
    ...
    def test_merge_textual_obs_annotations_functionality(self):
        """Test merge textual obs annotations functionality - verify merge function creates CSV."""
        all_pat_list = self.pat2vec_obj.all_patient_list

        def merge_textual_obs_annotations_data(patient_ids, config_obj, overwrite=True):
            """Merge all textual obs annotations data from database and raise ValueError if empty."""
            all_data = get_all_features(config_obj)

            if all_data.empty:
                raise ValueError(
                    "merge_textual_obs_annotations_data() returned empty DataFrame — no data found in database",
                )

            output_dir = os.path.join(self.PROJ_NAME, "outputs")
            os.makedirs(output_dir, exist_ok=True)
            merged_path = os.path.join(output_dir, "textual_obs_annotations_data.csv")
        ...
        merged_data = merge_textual_obs_annotations_data(
            all_pat_list,
            self.config_obj,
            overwrite=True,
        )
        ...
        output_dir = os.path.join(self.PROJ_NAME, "outputs")
        csv_path = os.path.join(output_dir, "textual_obs_annotations_data.csv")
