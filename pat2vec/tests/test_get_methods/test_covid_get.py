...
class TestCovidGet:
    """Stage-mirroring pytest for test_covid_get.ipynb."""
    ...
    def _start_elastic(self, elastic_container):
        ...
        cls.PROJ_NAME = "covid_test_project"
        cls.DB_FILENAME = "temp_covid_db.sqlite"
        cls.DB_PATH = os.path.join(cls.PROJ_NAME, "outputs", cls.DB_FILENAME)

        # Cleanup previous test outputs
        for dir_to_remove in ["covid_test_project"]:
            try:
        ...
        df_covid = (
            pd.concat(covid_dfs, ignore_index=True)
            if len(covid_dfs) > 1
            else covid_dfs[0]
        )
    ...
    # --- CHANGE 3: teardown_class removed — session fixture handles container.stop() ---
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("covid", False) is True
        ), "Covid option should be enabled in config"
    ...
    def test_3_index_population_and_verification(self):
        ...
        # Verify COVID observations were ingested
        es_count = self.cs.elastic.count(index="basic_observations")["count"]
        expected_covid_count = len(self.patient_ids) * 3  # 5 patients * 3 rows each
        assert (
            es_count >= expected_covid_count
        ), f"Expected at least {expected_covid_count} COVID observations, got {es_count}"
    ...
    def test_6_covid_data_retrieval(self):
        ...
        assert covid_data is not None, "Covid data should not be None"
        if isinstance(covid_data, list):
            assert len(covid_data) > 0, "Covid data list should not be empty"
            assert not covid_data[0].empty, "Covid DataFrame should not be empty"
        else:
            assert not covid_data.empty, "Covid DataFrame should not be empty"
    ...
    def test_merge_covid_data_functionality(self):
        ...
        merged_path = os.path.join(output_dir, "covid_data.csv")
        merged_data.to_csv(merged_path, index=False)

        assert os.path.exists(merged_path), f"Merged file should exist at {merged_path}"

        merged_csv = pd.read_csv(merged_path)
        assert not merged_csv.empty, "Merged COVID DataFrame should not be empty"
