...
class TestCoreResusGet:
    ...
    def _start_elastic(self, elastic_container):
        ...
        for pid in cls.patient_ids:
            ...
            core_resus_dfs.append(df)

        df_core_resus = (
            pd.concat(core_resus_dfs, ignore_index=True)
            if len(core_resus_dfs) > 1
            else core_resus_dfs[0]
        )
        df_core_resus = df_core_resus.where(pd.notnull(df_core_resus), None)
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("core_resus", False) is True
        ), "Core resus option should be enabled in config"
    ...
    def test_3_index_population_and_verification(self):
        ...
        # Verify core resus observations were ingested
        es_count = self.cs.elastic.count(index="observations")["count"]
        expected_core_resus_count = (
            len(self.patient_ids) * 3
        )  # 5 patients * 3 rows each
        assert (
            es_count >= expected_core_resus_count
        ), f"Expected at least {expected_core_resus_count} core resus observations, got {es_count}"
    ...
    def test_core_resus_data_retrieval(self):
        ...
        core_resus_data = get_core_resus(
        ...
        assert core_resus_data is not None, "Core resus data should not be None"
        if isinstance(core_resus_data, list):
            assert len(core_resus_data) > 0, "Core resus data list should not be empty"
            assert not core_resus_data[
                0
            ].empty, "Core resus DataFrame should not be empty"
        else:
            assert not core_resus_data.empty, "Core resus DataFrame should not be empty"
    ...
    def test_merge_core_resus_data_functionality(self):
        ...
        assert not merged_data.empty, "Merged Core Resus DataFrame should not be empty"
