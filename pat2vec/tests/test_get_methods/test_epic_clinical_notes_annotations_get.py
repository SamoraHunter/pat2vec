...
class TestEpicClinicalNotesAnnotationsGet:
    """Stage-mirroring pytest for test_epic_clinical_notes_get.ipynb."""
    ...
    def _start_elastic(self, elastic_container):
        ...
        cls.PROJ_NAME = "epic_clinical_notes_test_project"
        cls.DB_FILENAME = "temp_epic_clinical_notes_db.sqlite"
        cls.DB_PATH = os.path.join(cls.PROJ_NAME, "outputs", cls.DB_FILENAME)

        for dir_to_remove in ["epic_clinical_notes_test_project"]:
            try:
        ...
        cls.cs.elastic.indices.refresh(index="epic_clinical_notes")
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("epic_clinical_notes_annotations", False)
            is True
        ), "Epic clinical notes annotations option should be enabled in config"
    ...
    def test_3_index_population_and_verification(self):
        ...
        es_count = self.cs.elastic.count(index="epic_clinical_notes")["count"]
        expected_notes_count = len(self.patient_ids) * 3
        assert (
            es_count >= expected_notes_count
        ), f"Expected at least {expected_notes_count} clinical notes, got {es_count}"
    ...
    def test_epic_clinical_notes_annotations_vector_validation(self):
        """Verify pat_maker produced actual values in the epic_clinical_notes annotation feature vector.

        Catches the case where vectorisation silently fails — the DataFrame
        has columns but all values are null or empty.

        Epic clinical notes annotations use pattern:
        pretty_name_count_epic_clinical_notes_{pretty_name_value} where pretty_name_value
        is derived from the 'pretty_name' field in the annotations database (ann_epic_clinical_notes table).
        """
        all_features = get_all_features(self.config_obj)
        ...
        assert feature_values.sum() > 0, (
            "Feature extraction produced no meaningful numeric output. "
            "All epic_clinical_notes annotation features are zero or null after fillna."
        )
        ...
        for col in sorted(feature_cols):
            ...
            assert (
                len(values) > 0
            ), f"Feature column '{col}' should have at least one non-null value after feature extraction"
            assert (
                values.min() >= 0
            ), f"Feature column '{col}' should have non-negative counts, got min={values.min()}"
    ...
    def test_merge_epic_clinical_notes_documents_functionality(self):
        """Test document merge functionality - verify the post-processing merge
        function extracts all patients' epic clinical notes documents from the
        database (written by pat_maker) into a single dataframe.
        """
        ...
        assert merged_docs_path is not None, "Merged documents path should not be None"
        ...
        assert not merged_docs.empty, (
            ...
            "the pat2vec pipeline should have saved epic clinical notes "
            "documents to the database."
        )

    def test_merge_epic_clinical_notes_annotations_functionality(self):
        ...
        ), "Merged annotations path should not be None"
        assert os.path.exists(
            merged_annots_path,
