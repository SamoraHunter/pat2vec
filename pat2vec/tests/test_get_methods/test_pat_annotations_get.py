...
class TestPatAnnotationsGet:
    ...
    def test_merge_pat_annotations_functionality(self):
        """Test merge pat_annotations functionality - verify merge function creates CSV."""
        all_pat_list = self.pat2vec_obj.all_patient_list

        def merge_pat_annotations_data(patient_ids, config_obj, overwrite=True):
            """Merge all pat_annotations data from database and raise ValueError if empty."""
            all_data = get_all_features(config_obj)

            if all_data.empty:
                raise ValueError(
                    "merge_pat_annotations_data() returned empty DataFrame — no data found in database",
                )

            output_dir = os.path.join(self.PROJ_NAME, "outputs")
            os.makedirs(output_dir, exist_ok=True)
            merged_path = os.path.join(output_dir, "pat_annotations_data.csv")
        ...
        merged_data = merge_pat_annotations_data(
            all_pat_list,
            self.config_obj,
            overwrite=True,
        )
        ...
        output_dir = os.path.join(self.PROJ_NAME, "outputs")
        csv_path = os.path.join(output_dir, "pat_annotations_data.csv")
