...
"""
        ...
        assert os.path.exists(
            cls.creds_filename
        ), f"Credentials file should exist after creation at {cls.creds_filename}"
        print(
            f"After creation: Credentials file exists at {os.path.abspath(cls.creds_filename)}"
        )
        ...
        assert os.path.exists(
            cls.creds_filename
        ), f"Credentials file should exist after populate_elastic_with_dummy_data at {cls.creds_filename}"
        print(
            f"After populate: Credentials file exists at {os.path.abspath(cls.creds_filename)}"
        )

        # Set up CohStack client
        ...
        # Generate and ingest epr_documents data (required for annotations pipeline)
        ...
        # Create main config with annotations option enabled
        from pat2vec.util.config_pat2vec import config_class
        ...
        cls.config_obj = config_class(
            ...
            use_controls=False,
            medcat=True,  # Enable MedCAT for annotation generation
            start_time=None,
        ...
        # Run pat2vec pipeline to populate annotations via MedCAT
