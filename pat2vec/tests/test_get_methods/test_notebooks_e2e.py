...
def get_notebook_path(key: str) -> str:
    ...
    root_dir = "/workspaces/pat2vec/notebooks/test/"

    if key == "annotations_mrc":
        return f"{root_dir}test_annotations_mrc_get.ipynb"
    elif key == "negated_presence_annotations":
        return f"{root_dir}test_negated_presence_annotations_get.ipynb"
    elif key == "annotations_reports":
        return f"{root_dir}test_annotations_reports_get.ipynb"
    else:
        # All notebooks follow pattern: test_{key}_get.ipynb in root directory
        return f"{root_dir}test_{key}_get.ipynb"
