...
class TestNegatedPresenceAnnotationsGet:
    ...
    def test_merge_negated_presence_annotations_functionality(self):
    ...
    def test_negated_presence_annotations_vector_validation(self):
        """Verify pat_maker produced actual values in the epr annotation feature vector.

        Catches the case where vectorisation silently fails — the DataFrame
        has columns but all values are null or empty.

        This test uses negated presence filtering (filtering for Presence_Value: False),
        but the feature format is still pretty_name_count_epr_{pretty_name_value}
        """
        all_features = get_all_features(self.config_obj)
        ...
        feature_cols = [
            c for c in all_features.columns if c.startswith("pretty_name_count_epr_")
        ]

        assert len(feature_cols) > 0, (
            f"No epr annotation columns found. "
        ...
        assert len(totally_empty_cols) < len(feature_cols), (
            f"All epr annotation columns are empty - vectorisation is failing. "
            f"Null columns: {list(totally_empty_cols.index)}"
        )

        print(f"Found {len(feature_cols)} epr annotation feature columns")
