...
class TestNegatedPresenceAnnotationsGet:
    ...
    def test_negated_presence_annotations_vector_validation(self):
        """Verify pat_maker produced actual values in the negated feature vector.
        ...
        feature_cols = [c for c in all_features.columns if "negated" in c.lower()]

        assert len(feature_cols) > 0, (
            f"No negated annotation columns found. "
        ...
        assert len(totally_empty_cols) == 0, (
            f"The following negated annotation columns are entirely null after pat_maker ran:\n"
        ...
        print(f"Found {len(feature_cols)} negated annotation feature columns")
