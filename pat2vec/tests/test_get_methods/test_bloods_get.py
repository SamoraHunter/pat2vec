...
class TestBloodsGet:
    """Stage-mirroring pytest for test_bloods_get.ipynb."""
    ...
    def _start_elastic(self, elastic_container, tmp_path_factory):
        ...
        cls.TEMP_DIR = str(tmp_path_factory.mktemp("bloods_test_project"))
        cls.PROJ_NAME = "bloods_test_project"
        cls.DB_FILENAME = "temp_bloods_db.sqlite"
        ...
        temp_proj_dir_populate = os.path.join(cls.TEMP_DIR, "bloods_test_project")
        ...
        df_bloods = (
            pd.concat(bloods_dfs, ignore_index=True)
            if len(bloods_dfs) > 1
            else bloods_dfs[0]
        )
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("bloods", False) is True
        ), "Bloods option should be enabled in config"
    ...
    def test_3_index_population_and_verification(self):
        ...
        # Verify bloods observations were ingested
        es_count = self.cs.elastic.count(index="basic_observations")["count"]
        expected_bloods_count = len(self.patient_ids) * 3  # 5 patients * 3 rows each
        assert (
            es_count >= expected_bloods_count
        ), f"Expected at least {expected_bloods_count} bloods observations, got {es_count}"
    ...
    def test_bloods_data_retrieval(self):
        ...
        # Note: When batch_mode=True and pat_batch is empty, get_current_pat_bloods
        # returns a DataFrame with just the client_idcode column.

    def test_merge_bloods_data_functionality(self):
        """Test merge bloods data functionality - verify merge function creates CSV."""
        all_pat_list = self.pat2vec_obj.all_patient_list
