...
"""
        ...
        # Generate and ingest VTE status data
    ...
    def test_merge_vte_status_csv_functionality(self):
        ...
        # Check if credentials file exists (conftest cleanup may have removed it)
        if not os.path.exists(self.creds_filename):
            host, username, password = self.es_container.get_credentials()
            creds_content = f"""
username = "{username}"
password = "{password}"
api_key = None
