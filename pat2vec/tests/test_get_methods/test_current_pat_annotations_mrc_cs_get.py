...
class TestCurrentPatAnnotationsMrcCsGet:
...
"""
        ...
        df_annotations = (
            pd.concat(annotations_dfs, ignore_index=True)
            if len(annotations_dfs) > 1
            else annotations_dfs[0]
        )
        df_annotations = df_annotations.where(pd.notnull(df_annotations), None)
        ...
        cls.config_obj = config_class(
            ...
            credentials_path=cls.creds_filename,
            current_path_dir="",
            main_options={"annotations_mrc": True},
            batch_mode=True,
            verbosity=0,
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("annotations_mrc", False) is True
        ), "Annotations MRC option should be enabled in config"
    ...
    def test_3_index_population_and_verification(self):
        ...
        expected_annotations_count = len(self.patient_ids) * 3
        assert (
            es_count >= expected_annotations_count
        ), f"Expected at least {expected_annotations_count} annotations, got {es_count}"
    ...
    def test_annotations_mrc_data_retrieval(self):
        """Test annotations MRC data retrieval - verify features can be retrieved."""
        ...
        annotations_data = get_current_pat_annotations_mrc_cs(
            ...
            batch_mct_docs_annotations=pd.DataFrame(),
            config_obj=self.config_obj,
        )

        assert annotations_data is not None, "Annotations MRC data should not be None"
        if isinstance(annotations_data, list):
            assert (
                len(annotations_data) > 0
            ), "Annotations data list should not be empty"
            if len(annotations_data) > 0 and not annotations_data[0].empty:
                assert (
                    "client_idcode" in annotations_data[0].columns
                ), "DataFrame should have client_idcode column"
        else:
            assert (
                not annotations_data.empty
            ), "Annotations DataFrame should not be empty"

    def test_7_annotations_mrc_data_retrieval_with_real_docs(self):
        """Test annotations MRC data retrieval with real document batch."""
        from pat2vec.pat2vec_get_methods.get_method_current_pat_annotations_mrc_cs import (
            get_current_pat_annotations_mrc_cs,
        )
        ...
        client_idcode = all_pat_list[0]

        start_year_str = str(self.config_obj.global_start_year)
        start_month_str = str(self.config_obj.global_start_month).zfill(2)
        end_year_str = str(self.config_obj.global_end_year)
        end_month_str = str(self.config_obj.global_end_month).zfill(2)

        search_string = f"observationdocument_recordeddtm:[{start_year_str}-{start_month_str}-01T00:00:00 TO {end_year_str}-{end_month_str}-31T23:59:59]"
        ...
        query = {
            ...
            "query": {
                "bool": {
                    "filter": [{"term": {"client_idcode": client_idcode}}],
        ...
        annotations_data = get_current_pat_annotations_mrc_cs(
            current_pat_client_id_code=client_idcode,
            target_date_range=(2020, 1, 1, 2023, 12, 31),
            batch_mct_docs_annotations=batch_mct_docs_annotations,
            config_obj=self.config_obj,
        )
        ...
        if isinstance(annotations_data, list):
            if len(annotations_data) > 0:
                if not annotations_data[0].empty:
                    assert (
                        "client_idcode" in annotations_data[0].columns
                    ), "DataFrame should have client_idcode column"
                    assert len(annotations_data[0]) > 0, "DataFrame should not be empty"
        else:
            assert (
                not annotations_data.empty
            ), "Annotations DataFrame should not be empty"
