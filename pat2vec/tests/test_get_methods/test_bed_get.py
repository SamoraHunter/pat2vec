...
class TestBedGet:
    """Stage-mirroring pytest for test_bed_get.ipynb."""
    ...
    def setup_class(cls: type) -> None:
        ...
        cls.PROJ_NAME = "bed_test_project"
        cls.DB_FILENAME = "temp_bed_db.sqlite"
        ...
        for dir_to_remove in ["bed_test_project"]:
            try:
...
"""
        ...
        # Generate and ingest BED data
        from pat2vec.util.elasticsearch_methods import ingest_data_to_elasticsearch
        from pat2vec.util.get_dummy_data_cohort_searcher import generate_bed_data
        ...
        df_bed = (
            pd.concat(bed_dfs, ignore_index=True) if len(bed_dfs) > 1 else bed_dfs[0]
        )
    ...
    def test_2_config_and_pipeline_setup(self):
        ...
        assert (
            self.config_obj.main_options.get("bed", False) is True
        ), "BED option should be enabled in config"
    ...
    def test_3_index_population_and_verification(self):
        ...
        # Verify BED observations were ingested
        es_count = self.cs.elastic.count(index="observations")["count"]
        expected_bed_count = len(self.patient_ids) * 3  # 5 patients * 3 rows each
        assert (
            es_count >= expected_bed_count
        ), f"Expected at least {expected_bed_count} BED observations, got {es_count}"
