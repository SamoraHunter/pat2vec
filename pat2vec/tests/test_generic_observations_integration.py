...
class TestGenericObservationsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Generic Observations lifecycle:
    1. Synthetic data generation (custom search term).
    2. Ingestion of raw data into DB with sanitized table names.
    ...
    def setUp(self):
        self.test_patient_id = "P_GEN_OBS_TEST"
        ...
        # Custom search term that requires sanitization for DB table names
        self.custom_term = "Blood Pressure"
        self.safe_term = "blood_pressure"
        ...
        self.config = config_class(
            ...
            main_options={
                "obs": [self.custom_term],
            },
        )
        self.config.obs_time_field = (
    ...
    def test_generic_observations_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw generic observations)
        raw_obs_df = generate_observations_data(
            num_rows=3,
            entered_list=[self.test_patient_id],
            search_term=self.custom_term,
        ...
        time_field = getattr(self.config, "obs_time_field", "updatetime")
        raw_obs_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_obs_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_obs_blood_pressure)
        table_name = f"raw_obs_{self.safe_term}"
        save_raw_patient_batch(
            raw_obs_df, self.test_patient_id, table_name, self.config
        )
        db_raw_obs = get_df_from_db(
            self.config, "raw_data", table_name, patient_ids=[self.test_patient_id]
        )
        self.assertFalse(db_raw_obs.empty, "Raw Obs data should be in DB.")

        # 3. Feature Vectorization & Storage (Correct Date Range)
        # Signature: (current_pat_client_id_code, target_date_range, pat_batch, search_term, config_obj)
        features_inclusive = get_current_pat_obs(
        ...
        features_exclusive = get_current_pat_obs(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_obs,
            self.custom_term,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No features should have signal for exclusive date range.",
            )
