...
def test_get_epic_clinical_notes_appointments():
    """Test the get_epic_clinical_notes_appointments function with dummy data."""
    config = MockConfig()
    patient_id = "P123456"
    ...
    df = get_epic_clinical_notes_appointments(
    ...
    # Verify that binary features are extracted from clinical note appointment names
    assert any(col.startswith("epic_note_appt_name_") for col in df.columns) or df.empty
