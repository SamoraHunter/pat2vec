...
class TestBedIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Bed Status lifecycle:
    1. Synthetic data generation (Bed placement records).
    ...
    def setUp(self):
        self.test_patient_id = "P_BED_INTEGRATION_TEST"
    ...
    def test_bed_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Bed data)
        # Bed data is typically stored in the observations index.
        raw_bed_df = generate_observations_data(
            num_rows=3,
        ...
        # Bed Status data typically uses a configured time field or 'updatetime'.
        time_field = getattr(self.config, "bed_time_field", "updatetime")
        raw_bed_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_bed_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_bed)
        save_raw_patient_batch(raw_bed_df, self.test_patient_id, "raw_bed", self.config)
        db_raw_bed = get_df_from_db(
            self.config, "raw_data", "raw_bed", patient_ids=[self.test_patient_id]
        )
        self.assertFalse(db_raw_bed.empty, "Raw Bed data should be in DB.")
        self.assertEqual(len(db_raw_bed), 3)
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No Bed features should have signal for exclusive date range.",
            )
