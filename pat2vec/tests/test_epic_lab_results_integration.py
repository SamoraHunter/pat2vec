...
class TestEpicLabResultsIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Epic Lab Results lifecycle:
    1. Synthetic data generation (raw Epic Lab Results).
    ...
    def setUp(self):
        self.test_patient_id = "P_EPIC_LAB_TEST"
    ...
    def test_epic_lab_results_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Epic Lab Results)
        raw_lab_df = generate_epic_lab_results_data(
            num_rows=3,
        ...
        # Epic lab results typically use 'document_CollectedDate' for temporal filtering.
        time_field = "document_CollectedDate"
        raw_lab_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_lab_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_epic_lab_results)
        save_raw_patient_batch(
            raw_lab_df, self.test_patient_id, "raw_epic_lab_results", self.config
        )
        db_raw_lab = get_df_from_db(
            self.config,
            "raw_data",
            "raw_epic_lab_results",
            patient_ids=[self.test_patient_id],
        )
        self.assertFalse(db_raw_lab.empty, "Raw Epic Lab Results data should be in DB.")
        self.assertEqual(len(db_raw_lab), 3)
        ...
        features_inclusive = get_epic_lab_results(
        ...
        self.assertTrue(
            any(col.startswith("epic_lab_") for col in features_inclusive.columns),
            "Epic Lab feature columns should be present.",
        )
        ...
        features_exclusive = get_epic_lab_results(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_lab,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No Epic Lab features should have signal for exclusive date range.",
            )
