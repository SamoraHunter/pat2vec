...
class TestCoreResusIntegration(unittest.TestCase):
    """
    Comprehensive integration test for Core Resus lifecycle:
    1. Synthetic data generation (Resuscitation records).
    ...
    def setUp(self):
        self.test_patient_id = "P_RESUS_INTEGRATION_TEST"
        ...
        self.config.core_resus_time_field = (
    ...
    def test_core_resus_full_integration_lifecycle(self):
        # 1. Synthetic Data Generation (Raw Core Resus data)
        # Core Resus data is typically stored in the observations index.
        raw_resus_df = generate_observations_data(
            num_rows=3,
            entered_list=[self.test_patient_id],
            search_term="Resuscitation",
        ...
        # Core Resus data typically uses a configured time field or 'updatetime'.
        time_field = getattr(self.config, "core_resus_time_field", "updatetime")
        raw_resus_df[time_field] = [
            (self.base_date + timedelta(days=i - 2)).strftime("%Y-%m-%dT%H:%M:%S")
            for i in range(len(raw_resus_df))
        ]

        # 2. Ingestion of raw data into DB (raw_data_raw_resus)
        save_raw_patient_batch(
            raw_resus_df, self.test_patient_id, "raw_resus", self.config
        )
        db_raw_resus = get_df_from_db(
            self.config, "raw_data", "raw_resus", patient_ids=[self.test_patient_id]
        )
        self.assertFalse(db_raw_resus.empty, "Raw Core Resus data should be in DB.")
        self.assertEqual(len(db_raw_resus), 3)
        ...
        features_inclusive = get_core_resus(
        ...
        features_exclusive = get_core_resus(
            self.test_patient_id,
            self.target_date_range_exclusive,
            db_raw_resus,
            config_obj=self.config,
        ...
        if not features_exclusive.empty:
            ...
            self.assertTrue(
                ...
                "No Core Resus features should have signal for exclusive date range.",
            )
