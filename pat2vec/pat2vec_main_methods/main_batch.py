        ...
        if (current_pat_client_id_code not in stripped_list and already_done == False):
            ...
            try:
                ...
                p_bar_entry = current_pat_client_id_code + \
                    "_" + str(target_date_range)

                if main_options.get('demo'):
                    update_pbar(p_bar_entry, start_time,
                                0, 'demo', t, config_obj)
                    current_pat_demo = get_demo(
                        current_pat_client_id_code, target_date_range, batch_demo, config_obj=config_obj)
                ...
                if main_options.get('core_02'):
                    update_pbar(p_bar_entry, start_time, 1,
                                'core_02', t, config_obj)
                ...
                if main_options.get('vte_status'):
                    update_pbar(p_bar_entry, start_time, 3,
                                'vte_status', t, config_obj)
                ...
                pat_concatted.insert(0, 'client_idcode',
                                     current_pat_client_id_code)

                update_pbar(p_bar_entry, start_time, 2,
                            'saving...', t, config_obj)

                output_path = config_obj.current_pat_lines_path + current_pat_client_id_code + \
                    "/" + str(current_pat_client_id_code) + \
                    "_" + str(target_date_range)+".csv"
                ...
                if (remote_dump == False):
                    ...
                    if (multi_process == True):
                        ...
                        write_remote(output_path, pat_concatted,
                                     config_obj=config_obj)
                    else:
                ...
                try:
                    update_pbar(p_bar_entry, start_time, 2,
                                f'Done {len(pat_concatted.columns)} cols in {int(time.time() - start_time)}s, {int((len(pat_concatted.columns)+1)/int(time.time() - start_time)+1)} p/s', t, config_obj)
                except:
                    update_pbar(p_bar_entry, start_time, 2,
                                f'Columns n={len(pat_concatted.columns)}', t, config_obj)
                    pass
