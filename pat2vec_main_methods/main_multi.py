...
from pat2vec.util.methods_get import enum_target_date_vector, list_dir_wrapper, update_pbar
...
def main_multi(arg_list):
    ...
    main_options = arg_list[2]  # append to arg list on function call.
    ...
    if (current_pat_client_id_code+"_"+str(target_date_range) not in stripped_list_start):
        ...
        stripped_list = [x.replace(".csv", "")
                         for x in list_dir_wrapper(current_pat_lines_path)]
        ...
        if (current_pat_client_id_code + "_" + str(target_date_range) not in stripped_list):
            ...
            try:
                ...
                update_pbar(p_bar_entry, start_time, 0,
                            'demo', t=t, config_obj=config_obj)
                ...
                update_pbar(p_bar_entry, start_time, 2,
                            'bloods', t=t, config_obj=config_obj)
                ...
                update_pbar(p_bar_entry, start_time, 3,
                            'drugs', t=t, config_obj=config_obj)
                ...
                update_pbar(p_bar_entry, start_time, 4,
                            'diagnostics', t=t, config_obj=config_obj)
                ...
                update_pbar(p_bar_entry, start_time, 1,
                            'core_02', t=t, config_obj=config_obj)
                ...
                update_pbar(p_bar_entry, start_time, 3,
                            'vte_status', t=t, config_obj=config_obj)
                ...
                update_pbar(p_bar_entry, start_time, 4,
                            'hosp_site', t=t, config_obj=config_obj)
                ...
                update_pbar(p_bar_entry, start_time, 1,
                            'core_resus', t=t, config_obj=config_obj)
                ...
                update_pbar(p_bar_entry, start_time, 2,
                            'news', t=t, config_obj=config_obj)
                ...
                update_pbar(p_bar_entry, start_time, 2,
                            'concatenating', t=t, config_obj=config_obj)
                ...
                if (remote_dump == False):
                    ...
                    pat_concatted.to_csv(
                        current_pat_line_path + str(current_pat_client_id_code) + "_" + str(target_date_range)+".csv")
                else:
                ...
                update_pbar(p_bar_entry, start_time, 2,
                            f'Done {len(pat_concatted.columns)} cols in {int(time.time() - start_time)}s, {int(len(pat_concatted.columns)/int(time.time() - start_time))} p/s', t=t, config_obj=config_obj)
